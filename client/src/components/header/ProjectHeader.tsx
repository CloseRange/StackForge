...
export const ProjectHeader = ({
  ...
  const renderTabButton = (tab: (typeof TABS)[number], compact: boolean) => {
      ...
      <button
        ...
        className={`btn-motion btn-shimmer group relative overflow-hidden rounded-xl border py-1.5 text-sm font-semibold transition-all duration-300 ${
          isActive
            ...
            : isDarkMode
              ? `border-white/5 bg-white/[0.03] ${compact ? "px-2.5 sm:px-3" : "px-3.5"} text-slate-300 hover:border-white/20 hover:bg-white/[0.08] hover:text-white`
              : `border-slate-200 bg-white/70 ${compact ? "px-2.5 sm:px-3" : "px-3.5"} text-slate-600 hover:border-slate-300 hover:bg-white hover:text-slate-900`
        }`}
      >
        <span
        ...
        <span className="relative flex items-center">
          ...
          {compact ? (
            ...
            <span className="ml-1.5 whitespace-nowrap text-sm">{tab.label}</span>
          )}
        </span>
        {isActive ? (
          <span
  ...
  return (
    <header className={`sticky top-0 z-40 border-b backdrop-blur-md ${
      ...
      <div className="mx-auto max-w-[1600px] px-3 py-2 sm:px-4 sm:py-2.5 md:px-6">
        <div className="flex items-center justify-between gap-2 sm:gap-3 md:grid md:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] md:gap-4">
          ...
          {/* md–xl: compact (icon-only unless active) */}
          <nav
            className={`relative hidden md:flex xl:hidden items-center justify-center gap-1 overflow-x-auto rounded-2xl border p-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden ${
              isDarkMode
          ...
          {/* xl+: full labels always visible */}
          <nav
            className={`relative hidden xl:flex items-center justify-center gap-1 overflow-x-auto rounded-2xl border p-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden ${
              isDarkMode
                ? "border-white/10 bg-black/20 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]"
                : "border-slate-200 bg-slate-100/80 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)]"
            }`}
          ...
          <div className="ml-auto flex shrink-0 items-center gap-1.5 sm:gap-3 md:ml-0 md:justify-self-end">
            ...
            {showSettings ? (
              <button
                ...
                className={`btn-motion btn-shimmer hidden h-9 w-9 items-center justify-center rounded-xl border text-sm font-medium transition lg:flex ${
                  isDarkMode
        ...
        <nav
          className={`relative mt-2 flex w-full items-center justify-center gap-1 overflow-x-auto rounded-2xl border p-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden md:hidden ${
            isDarkMode
              ? "border-white/10 bg-black/20 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]"
              : "border-slate-200 bg-slate-100/80 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)]"
          }`}
